...
public class UserController {
	...
	@PatchMapping(value = "user/{userId}", headers = "Accept=application/json")
	public ResponseEntity<User> updateUserPartial(@PathVariable("id") int userId, @RequestBody User currentUser) {
		Optional<User> userOptional = userService.getUser(userId);
		if (!userOptional.isPresent()) {
			return new ResponseEntity<User>(HttpStatus.NOT_FOUND);
		}

		userService.updatePartially(userOptional.get(), userId);
